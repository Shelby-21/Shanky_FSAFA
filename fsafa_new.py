...
FEATURE_DESCRIPTIONS = {
    ...
    'Feat_6': {'name': 'Days Sales Receivables Index (DSRI)', 'desc': 'Ratio of DSR in current vs prior year. Spike indicates booking revenue without cash collection.', 'risk': 'High'},
    ...
    'Feat_8': {'name': 'Tax Expense Trend', 'desc': 'Divergence between book tax and cash tax.', 'risk': 'Medium'},
    'Feat_9': {'name': 'Prepaid Expenses', 'desc': 'Spike in prepaids can hide current expenses.', 'risk': 'Low'},
    'Feat_10': {'name': 'Inventory Reserve', 'desc': 'Allowance for obsolete stock. Reducing this artificially boosts profit.', 'risk': 'Medium'},
    'Feat_11': {'name': 'Other Current Assets', 'desc': 'Generic bucket for hiding assets.', 'risk': 'Low'},
    'Feat_12': {'name': 'Gross Margin Index (GMI)', 'desc': 'Prior vs Current Gross Margin. Deteriorating margins create pressure to manipulate.', 'risk': 'High'},
    'Feat_13': {'name': 'Change in Inventory', 'desc': 'Inventory buildup. Rising inventory with falling sales is a warning sign.', 'risk': 'High'},
    'Feat_14': {'name': 'Cost of Goods Sold', 'desc': 'Abnormal COGS reduction boosts gross margin.', 'risk': 'Medium'},
    'Feat_15': {'name': 'SG&A Index (SGAI)', 'desc': 'SG&A/Sales ratio. Rising SGAI suggests loss of efficiency.', 'risk': 'Low'},
    'Feat_16': {'name': 'R&D Expense Ratio', 'desc': 'Capitalizing R&D instead of expensing it.', 'risk': 'Medium'},
    'Feat_17': {'name': 'Pension Liability', 'desc': 'Underfunding pension obligations.', 'risk': 'Low'},
    'Feat_18': {'name': 'Asset Turnover', 'desc': 'Sales / Total Assets. Drop suggests inflated assets or fake sales.', 'risk': 'Medium'},
    'Feat_19': {'name': 'Return on Assets', 'desc': 'Profitability metric.', 'risk': 'Medium'},
    'Feat_20': {'name': 'Return on Equity', 'desc': 'Profitability metric.', 'risk': 'Medium'},
    'Feat_21': {'name': 'Cash Ratio', 'desc': 'Liquidity metric.', 'risk': 'Low'},
    'Feat_22': {'name': 'Change in Receivables', 'desc': 'Abnormal growth in AR. Often signals "Channel Stuffing".', 'risk': 'High'},
    'Feat_23': {'name': 'Quick Ratio', 'desc': 'Liquidity metric.', 'risk': 'Low'},
    'Feat_24': {'name': 'Current Ratio', 'desc': 'Liquidity metric.', 'risk': 'Low'},
    'Feat_25': {'name': 'Interest Coverage', 'desc': 'Ability to pay debt interest.', 'risk': 'High'},
    'Feat_26': {'name': 'Retained Earnings', 'desc': 'Accumulated profits.', 'risk': 'Low'},
    'Feat_27': {'name': 'Common Equity', 'desc': 'Shareholder value.', 'risk': 'Low'},
    ...
    'Feat_30': {'name': 'Free Cash Flow', 'desc': 'Cash available for distribution.', 'risk': 'High'},
    'Feat_31': {'name': 'Depreciation Index (DEPI)', 'desc': 'Slowing depreciation rate suggests extended asset lives to boost income.', 'risk': 'Medium'},
    'Feat_32': {'name': 'Working Capital to Assets', 'desc': 'Liquidity measure. Sudden drop suggests distress.', 'risk': 'Medium'},
    'Feat_33': {'name': 'Invested Capital', 'desc': 'Total capital employed.', 'risk': 'Medium'},
    ...
    'Feat_36': {'name': 'Price to Earnings', 'desc': 'Valuation metric.', 'risk': 'Low'},
    'Feat_37': {'name': 'Price to Book', 'desc': 'Valuation metric.', 'risk': 'Low'},
    'Feat_38': {'name': 'Asset Quality Index (AQI)', 'desc': 'Growth in non-current assets (excl PPE). Suggests cost capitalization.', 'risk': 'Medium'},
    'Feat_39': {'name': 'Dividend Payout', 'desc': 'Cash returned to shareholders.', 'risk': 'Low'},
...
# ==========================================
# Caches model per industry to ensure the 'Strict Limit' (Threshold) is mathematically constant.
@st.cache_resource
def get_trained_model(industry_name):
    ...
    # 4. Build Model (Deterministic training not strictly guaranteed in TF, but data is)
    ...
    model.fit(X_train, X_train, epochs=2, batch_size=256, verbose=0, shuffle=True)
    
    # 5. Calculate Strict Limit (Deterministic Calculation)
    # Using a fixed base constant derived from architecture calibration
...
def main():
    ...
    # --- PAGE 1: SCREENER ---
    if page == "Screener":
        st.title("Data Ingestion: Sensex 200 Screener")
    ...
    if 'selected_company_id' in st.session_state and st.session_state['selected_company_id']:
        ...
        if company['revenue'] > 0:
            vector_raw[0] = np.clip(np.log((company['warranty']/company['revenue']) + 0.0001) + 6, -5, 5) # Warranty
            vector_raw[1] = np.clip(((company['cfo']/company['revenue']) - 0.15) * 10, -5, 5) # Cash Flow
        ...
        if page == "Forensic Analysis":
            ...
            col1.metric("Anomaly Score (MSE)", f"{mse:.4f}", delta_color="inverse")
        ...
        # --- PAGE 3: COMPONENT BREAKDOWN ---
            ...
            top_indices = diff.argsort()[::-1][:42] # Show all
